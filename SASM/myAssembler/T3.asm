...
#data1的后12位代表右半边LED应显示的数据,data2的前12位代表左半边应显示的数据
#举个例子来说明：假设LED共有4个,等分成2组,每组2个LED,
#用类似001100的数据初始化,假设右边LED只有2个,则每次都取数据的后2位,这样001100一开始是00,
#在半秒后001100逻辑右移1位变成000110,后两位就变成了10,之后再隔半秒就变成了11,01,00...变到全0时就代表完整的执行了一次模式1
mode1:
#s1存右边的数据,s2存左边的数据,s3存移位的计数(为0时回到mode1标签),s4存放等待半秒的计数
	lw $s1,data1
	lw $s2,data2
...
wait:	#子函数,用于延时半秒
	addi $s4,$zero,0x032	#延时半秒的循环次数存入寄存器s4

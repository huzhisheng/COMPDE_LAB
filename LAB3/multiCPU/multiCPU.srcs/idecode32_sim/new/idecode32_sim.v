...
module idecode32_sim();
    ...
    idecode32 u_idecode32(.clk(clock),.ins(ins),.ALU_out(ALU_out),.PC_DO(PC_DO),.DM_RD(DM_RD),.RFWr(RFWr),.EXTOp(EXTOp),.WRSel(WRSel),.WDSel(WDSel),.A(rf_rd1),.B(rf_rd2),.E(ext),.debug_A3(debug_A3),.debug_WD(debug_WD));
